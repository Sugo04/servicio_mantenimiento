...
class ServicioMantenimiento(models.Model):
    ...
    customer_id = fields.Many2one(
        "res.partner", string="Cliente", related="garantia_id.customer_id", store=True
    )
    product_id = fields.Many2one(
        "product.product",
        string="Producto",
        related="garantia_id.product_id",
        store=True,
    )
    fecha_solicitud = fields.Date(
        string="Fecha de Solicitud", default=fields.Date.context_today, required=True
    )
    ...
    def create(self, vals):
        ...
        if garantia.status != "valid":
            raise UserError(
                "No se puede crear un mantenimiento para una garantía expirada."
            )
